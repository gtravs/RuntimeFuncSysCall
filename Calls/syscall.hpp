...
//  [[nodiscard]] uintptr_t GetStubAddr(std::string_view stub_name);
  ...
  [[nodiscard]] uint32_t GetSyscallNumber(uint32_t function_name_hashed);
  ...
  template<typename... ServiceArgs>
  FunctionResult<NtStatus> DirectCallSyscall(uint32_t function_name_hashed, ServiceArgs... args) {
    uint32_t syscall_no = GetSyscallNumber(function_name_hashed);
  ...
  template<typename... ServiceArgs>
  NtStatus CallSyscall(uint32_t function_name_hashed, ServiceArgs... args) {
      uint32_t syscall_no;
      uintptr_t stub_addr;
      uintptr_t syscall_inst_addr;

      syscall_no = GetSyscallNumber(function_name_hashed);
